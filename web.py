...
@app.route("/")
def index():
    link = "<h1>歡迎加入林煥杰的網站首頁</h1>"
    link += "<a href='/mis'>課程</a><hr>"
    link += "<a href='/today'>今天日期</a><hr>"
    link += "<a href='/about'>關於煥杰</a><hr>"
    link += "<a href='/welcome?nick=煥杰&dep=靜宜資管'>GET傳值</a><hr>"
    ...
    link += "<a href='/rate'>本週新片DB更新</a><hr>"
    return link
...
def weather():
    ...
    if request.method == "POST":
        ...
        Data = requests.get(url, verify=False)  # 加上 verify=False 以防止 SSL 驗證問題
...
def welcome():
    user = request.values.get("nick", "林煥杰")
